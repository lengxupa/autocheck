...
def write_sublmp(seconds,i,j,aa,co,b,med,ifile):
	...
	src.write("#PBS -N %s%s%s%s%s%sAA\n" %(med[:3],b[:4],co,aa[:3],(j[:2]+j[-1]),i[-3:]))
	src.write("#PBS -q %s\n" %a)
	src.write("\n")
	src.write("set echo\n")
	src.write("\n")
...
def subcontinue(po,fi,ta,co,te,rt,jud):
	...
	if not os.path.exists(new):
		...
		ste2=50000
		meltcontinue('melt.in',('melt%s.in' %new),t,ste2,te)
		if jud:
...
def meltcontinue(ifile,ofile,t,ste,te):
	...
	for line in src.readlines():
		ln=line.split()
		if ln and (ln[0]=='velocity' or ln[0]=='#velocity') and ln[2]=='create':
			ln[0]='velocity'
			ln[3]=te[-3:]
		if ln and ln[0]=='dump':
			if dump:
				des.write('dump 1 all atom 100 mdz.dump\n')
				des.write('dump            d1 all custom %d extension.dump id type x y z vx vy vz fx fy fz\n' %ste)
				des.write('dump 3 all atom %d md.*.dump\n' %ste)
				des.write('dump 4 all xyz %d x.xyz\n' %ste)
				dump=0
			continue
...
def dumpfiles(j):
	if os.path.exists('dumpfiles'):
		os.system('rm -r dumpfiles')
	if not os.path.exists('dumpfiles'):
		os.mkdir('dumpfiles')
		ns=['5ns','10ns','20ns','50ns','100ns','200ns','500ns']
		sta=eval(ns[ns.index(j)-1][:-2])*1000000 if ns.index(j) else 0
		ter=eval(j[:-2])*1000000
		ste=(ter-sta)/100
		dumprange=range(((sta/ste+(1 if sta%ste else 0))*ste),(ter+(ste if not ter%ste else 0)),ste)
		for k in dumprange:
			if os.path.exists('md.%d.dump' %k):
				os.system('cp md.%d.dump dumpfiles' %k)


suc=open('check/checkekksuctemp.txt','w')
fai=open('check/checkekkfaitemp.txt','w')
fixlist=['fix','yfix']
PEKK=[str(i) for i in (range(440,820,40)+[660,700,740])]
for i in ['PEKK']:
	for j in fixlist:
		...
		for root, dirs, files in os.walk(cwd, topdown=False):
			rt=root.split('/')
			if rt[-1]=='coollong' or (rt[-1]=='2500000' and rt[-2]=='coollong'):
				if 'step4.data' in files:
					os.chdir(root)
					tmp=os.getcwd()
					for Root, Dirs, Files in os.walk(tmp, topdown=False):
						Rt=Root.split('/')
						if len(Rt)>len(rt) and Rt[rt.index(rt[-1])+1][0]=='V' and (Rt[-1][0]=='V' or Rt[-1][-2:]=='ns'):
							if Rt[rt.index(rt[-1])+1][-3:] not in PEKK:
								continue
							if rt[8]=='short':
								continue
							if ('step4.data' in Files):
								print i,j,rt[8],(rt[-1][:3]+rt[-1][-1]),Rt[rt.index(rt[-1])+1],(Rt[-1] if Rt[-1][-2:]=='ns' else '5ns'),'success'
								os.chdir(Root)
								dumpfiles(Rt[-1] if Rt[-1][-2:]=='ns' else '5ns')
								if subcontinue(i,j,rt[8],(rt[-1][:3]+rt[-1][-1]),Rt[rt.index(rt[-1])+1],Rt[-1],checkfile('step4.data')):
									suc.write(' '.join([i,j,rt[8],(rt[-1][:3]+rt[-1][-1]),Rt[rt.index(rt[-1])+1],(Rt[-1] if Rt[-1][-2:]=='ns' else '5ns'),'success'])+'\n')
									suc.write(Root+'\n')
							else:
								a=1
								for k in Files:
									if '.o' in k:
										a=0
										print i,j,rt[8],(rt[-1][:3]+rt[-1][-1]),Rt[rt.index(rt[-1])+1],(Rt[-1] if Rt[-1][-2:]=='ns' else '5ns'),'failed'
										os.chdir(Root)
										if subcontinue(i,j,rt[8],(rt[-1][:3]+rt[-1][-1]),Rt[rt.index(rt[-1])+1],Rt[-1],1):
											os.chdir(Root)
											fai.write(' '.join([i,j,rt[8],(rt[-1][:3]+rt[-1][-1]),Rt[rt.index(rt[-1])+1],(Rt[-1] if Rt[-1][-2:]=='ns' else '5ns'),'failed'])+'\n')
											ERR=open(k).readlines()[-1]
											fai.write((ERR if ERR[:5]=='ERROR' else 'timeout\n'))
											fai.write(Root+'\n')
										break
								if a:
									print i,j,rt[8],(rt[-1][:3]+rt[-1][-1]),Rt[rt.index(rt[-1])+1],(Rt[-1] if Rt[-1][-2:]=='ns' else '5ns'),'continuing'
		os.chdir(cwd)
		os.chdir('../..')
suc.close()
fai.close()
