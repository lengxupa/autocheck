...
for i in ['PEKK']:
	for j in fixlist:
		if not os.path.exists('%s/test/structure/form1/%s' %(i,j)):
			continue
		os.chdir('%s/test/structure/form1/%s' %(i,j))
		cwd = os.getcwd()
		for root, dirs, files in os.walk(cwd, topdown=False):
			rt=root.split('/')
			if (rt[-1]=='Gas') or (rt[-1]=='Qeq'):
				if rt[-1]=='Gas':
					continue
				print 'sub',i,j,rt[-1]
				os.chdir(root)
